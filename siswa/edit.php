<?php
// siswa/edit.php - Edit Student Form & Handler
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nomor_peserta) || empty($nama_siswa) || empty($jk) || empty($tempat_lahir) || empty($tanggal_lahir) || empty($alamat) || empty($asal_sekolah) || empty($no_hp) || empty($tahun_ajaran)) {
        ...
        try {
            // Check if nomor_peserta is used by another student
            $check_stmt = $pdo->prepare("SELECT COUNT(*) FROM siswa WHERE nomor_peserta = ? AND id_siswa != ?");
            $check_stmt->execute([$nomor_peserta, $id_siswa]);
            if ($check_stmt->fetchColumn() > 0) {
                set_flash_message('error', 'Nomor peserta sudah terdaftar pada siswa lain!');
            } else {
                $update_stmt = $pdo->prepare("
                    UPDATE siswa 
                    SET nomor_peserta = ?, nama_siswa = ?, jk = ?, tempat_lahir = ?, tanggal_lahir = ?, alamat = ?, asal_sekolah = ?, no_hp = ?, tahun_ajaran = ?
                    WHERE id_siswa = ?
                ");
                $update_stmt->execute([$nomor_peserta, $nama_siswa, $jk, $tempat_lahir, $tanggal_lahir, $alamat, $asal_sekolah, $no_hp, $tahun_ajaran, $id_siswa]);
...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-md-6">
            <h2 class="fw-bold m-0"><i class="fa-solid fa-user-pen text-primary me-2"></i> Edit Siswa</h2>
            <p class="text-muted m-0">Perbarui biodata calon siswa baru</p>
        </div>
    ...
    <div class="card card-custom">
        <div class="card-header-custom">
            <span class="fs-5 text-dark fw-bold">Form Edit Biodata: <?= sanitize($siswa['nama_siswa']); ?></span>
        </div>
        <div class="card-body p-4">
            <form action="<?= base_url('siswa/edit.php?id=' . $siswa['id_siswa']); ?>" method="POST">
                <div class="row">
                    <!-- Left Column -->
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label for="nomor_peserta" class="form-label fw-semibold">Nomor Peserta <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="nomor_peserta" name="nomor_peserta" value="<?= sanitize($siswa['nomor_peserta']); ?>" required>
                        </div>
                        
                        <div class="mb-3">
                            <label for="nama_siswa" class="form-label fw-semibold">Nama Lengkap Siswa <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="nama_siswa" name="nama_siswa" value="<?= sanitize($siswa['nama_siswa']); ?>" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold d-block">Jenis Kelamin <span class="text-danger">*</span></label>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="jk" id="jk_l" value="L" <?= $siswa['jk'] === 'L' ? 'checked' : ''; ?> required>
                                <label class="form-check-input-label" for="jk_l">Laki-laki</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="jk" id="jk_p" value="P" <?= $siswa['jk'] === 'P' ? 'checked' : ''; ?> required>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" id="tempat_lahir" name="tempat_lahir" value="<?= sanitize($siswa['tempat_lahir']); ?>" required>
                            </div>
                            <div class="col-md-6 mb-3">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="asal_sekolah" name="asal_sekolah" value="<?= sanitize($siswa['asal_sekolah']); ?>" required>
                        </div>

                        <div class="mb-3">
                            <label for="no_hp" class="form-label fw-semibold">No HP/WhatsApp <span class="text-danger">*</span></label>
                            <input type="tel" class="form-control" id="no_hp" name="no_hp" value="<?= sanitize($siswa['no_hp']); ?>" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="tahun_ajaran" name="tahun_ajaran" required>
                                ...
                                for ($i = 0; $i < 4; $i++) {
                                    ...
                                    $selected = ($siswa['tahun_ajaran'] === $val) ? 'selected' : '';
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="alamat" name="alamat" rows="3" required><?= sanitize($siswa['alamat']); ?></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="text-end">
                    <button type="submit" class="btn btn-primary-custom">
                        <i class="fa-solid fa-floppy-disk me-2"></i> Perbarui Data Siswa
                    </button>
                </div>
            </form>
        </div>
